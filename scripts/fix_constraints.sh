...
VALUES ('a5f1b662-a367-4c5c-9c48-4097bbd8be4c', 'Send Notification', 'notification_user', 'notification', 45, 17, '{\"recipient\":\"{{\\\$trigger.user}}\",\"subject\":\"User login {{\\\$trigger.status}} on {{\\\$accountability.origin}}\",\"message\":\"We would like to confirm recent sign-in activity on your account.\n\nfrom IP: {{ \\\$accountability.ip }}\n\nwith User Agent: {{ \\\$accountability.userAgent }}\n\n**status:  {{\\\$trigger.status}}**\n\n[CISO360.AI](https://ciso360.ai/)\",\"permissions\":\"\\\$full\"}', NULL, NULL, 'b4bf500b-669c-48cd-bdf8-df3f21182127', '2022-06-05 01:27:42.271+00', NULL);

INSERT INTO directus_operations (id, name, key, type, position_x, position_y, options, resolve, reject, flow, date_created, user_created) 
VALUES ('a2be8de9-7233-4341-a044-69bf37a47b28', 'Send Email', 'mail_login', 'mail', 21, 1, '{\"to\":[\"{{\\\$trigger.payload.email}}\",\"alerts@altersec.com\"],\"subject\":\"User login {{\\\$trigger.status}} on {{\\\$accountability.origin}}\",\"body\":\"Email: {{\\\$trigger.payload.email}}\n\nfrom IP: {{ \\\$accountability.ip }}\n\nwith User Agent: {{ \\\$accountability.userAgent }}\n\n**status:  {{\\\$trigger.status}}**\"}', NULL, 'a5f1b662-a367-4c5c-9c48-4097bbd8be4c', 'b4bf500b-669c-48cd-bdf8-df3f21182127', '2022-06-05 01:27:42.271+00', NULL);
...
$PWD/scripts/regular_domain_hibp_scan.sh
$PWD/scripts/regular_email_scan.sh
